...
class WeatherScanner(Thread):
	...
	def __init__(self,alarm_config_dictionary,model):
		...
		#self.forecast_url = 'http://api.wunderground.com/api/' + alarm_config_dictionary["wunderground api key"] + '/geolookup/forecast/q/'+alarm_config_dictionary["wunderground location"]+'.json'
		self.start()
		...
		while(True):
			...
			if not(network_is_alive==is_alive):
				...
				if network_is_alive:
					...
					msg="internet_off"
				eventQ.put([dispatcher.send,{"signal":"Fault Update", "sender":dispatcher.Any,"msg":msg }])	
...
class AbstractState():
	...
	def set_state(self,state):
		...
		eventQ.put([dispatcher.send,{"signal":"Alarm Mode Update Model", "sender":dispatcher.Any}])
...
class AlarmModel():
	...
	def __str__(self):
		...
		string += "Last sensor triggered: "+ str(self.last_trig_sensor)+" when in state: "+str(self.last_trig_state)+"\n"
		...
		if key == "":	#System tic
			if self.input_activity>0:
				...
				if self.input_activity==0:
					self.input_string = ""
					self.display_string = ""
					eventQ.put([dispatcher.send,{"signal":"Input String Update Model", "sender":dispatcher.Any, "msg":self.display_string }])
			return
	...
	def update_sensor(self,sensor):
		...
		eventQ.put([dispatcher.send,{"signal":"Sensor Update Model", "sender":dispatcher.Any, "msg":sensor}])
	...
	def broadcast_message(self,msg):
		...
		eventQ.put([dispatcher.send,{"signal":"Alarm Message", "sender":dispatcher.Any, "msg":msg }])	
...
class AlarmController():
	...
	def __init__(self):
		...
		dispatcher.connect(self.handle_update_fault, signal="Fault Update", sender=dispatcher.Any,weak=False)
...
class Sensor(Thread):
	...
	def __init__(self, controller, config, dedicated_thread=False):
		...
		[self.pin,self.name,self.icon,self.pin_mode,period,self.normally_closed,self.sensor_type,armed_states,disarming_setting,self.play_sound] = config
	...
	def __str__(self):
		return "Sensor("+self.name+", polling_period: "+str(self.polling_period)+", normally closed: "+str(self.is_normally_closed())+", reading: " +str(self.get_reading()) +", locked: "+str(self.is_locked())+", armed: "+str(self.is_armed())+", grace: "+str(self._disarming_grace)+")"
...
class PiezoView():
	...
	def __init__(self,alarm_config_dictionary,model):
		...
		"""subscribe to several topics of interest (model)"""
		dispatcher.connect( self.alarm_mode, signal="Alarm Mode Update Model", sender=dispatcher.Any, weak=False)
		...
		dispatcher.connect( self.sensor_change, signal="Sensor Update Model", sender=dispatcher.Any, weak=False)
	...
	def sensor_change(self,msg):
		...
		if sensor.play_sound=="play_sound":
			...
			self.player.next([self.player.play_notes,{"string":[[880,50,0.12],[880,0,0.1],[880,50,0.12],[880,0,0.07],[880,50,0.12],[880,0,0]]}])	
	...
	def grace_timer(self,msg):
		if isinstance(self.alarm_mode,StateArming):
			if msg > 10:
				...
				self.player.next([self.player.play_notes,{"string":[[200,50,0.1],[200,0,0.2],[200,50,0.1],[200,0,0]]}])
		else:
	...
	def alarm_mode(self):
		...
		if isinstance(self.alarm_mode,StateAlert):
			...
			self.player.next([self.player.play_notes,{"string":[[440,50,0.1],[440,0,0.05],[440,50,0.1],[440,0,0]]}])
		elif isinstance(self.alarm_mode,StatePartiallyArmed):
			self.player.next([self.player.play_notes,{"string":[[440,50,0.1],[440,0,0.05],[503,50,0.1],[503,0,0.05],[566,50,0.1],[566,0,0]]}])
...
class LCDView():
	def __init__(self,alarm_config_dictionary,model):
		...
		try:
			self.lcd_backlight_timer_setting = alarm_config_dictionary["lcd_backlight_timer"]	#Timer setting to deactivate backlight when LCD is inactive.
		except:
		...
		self.table = string.maketrans(chr(128)+chr(129)+chr(130)+chr(131)+chr(132)+chr(133)+chr(134)+chr(135)+chr(0)+chr(1)+chr(2)+chr(3)+chr(4)+chr(5)+chr(6)+chr(7), "                ")
	...
	def init_screen(self):
		...
		try:
			...
			self.table = string.maketrans(self.get_char("clock")+self.get_char("door")+self.get_char("patio")+self.get_char("motion")+self.get_char("deg")+self.get_char("camera")+self.get_char("arrow"), "cDPMoCa")
	...
	def exit(self):
		try:
			...
			dispatcher.disconnect( self.update_weather, signal="Weather Update Model", sender=dispatcher.Any ,weak=False)
			dispatcher.disconnect( self.update_time, signal="Time Update Model", sender=dispatcher.Any ,weak=False)
			dispatcher.disconnect( self.update_alarm_mode, signal="Alarm Mode Update Model", sender=dispatcher.Any,weak=False )
			dispatcher.disconnect( self.update_fault, signal="Fault Update Model", sender=dispatcher.Any,weak=False)
			dispatcher.disconnect( self.update_PIN, signal="Input String Update Model", sender=dispatcher.Any ,weak=False)
			dispatcher.disconnect( self.update_msg, signal="Alarm Message", sender=dispatcher.Any ,weak=False)
			dispatcher.disconnect( self.update_sensor_state, signal="Sensor Update Model", sender=dispatcher.Any, weak=False)
			dispatcher.disconnect( self.update_grace_timer, signal="Grace Update Model", sender=dispatcher.Any, weak=False)
			dispatcher.disconnect( self.exit, signal="Terminate", sender=dispatcher.Any, weak=False)
		except IOError:
			pass
...
class SMSView():
	...
	def update_alarm_mode(self):
		...
		if isinstance(self.model.alarm_mode, StateAlert):
			self.last_alarm_mode_event = self.insert_event("RPI Intrusion", "System in Alert state\nSensor triggered: " + self.model.last_trig_sensor.name + " while in state " + str(self.model.last_trig_state)) #sends sms
		elif isinstance(self.model.alarm_mode, StateFire):
			self.last_alarm_mode_event = self.insert_event("RPI Fire", "System in Fire state\nFire detector triggered while in state " + str(self.model.last_trig_state)) #sends sms
...
class EmailView():
	...
	def update_alarm_mode(self):
		if isinstance(self.model.alarm_mode, StateAlert):
			self.create_email("RPI_Intrusion", "Sensor triggered: " + self.model.last_trig_sensor.name + " while in state " + str(self.model.last_trig_state)) #sends sms
		elif  isinstance(self.model.alarm_mode, StateFire):
			self.create_email("RPI_Fire", "Fire detector triggered while in state " + str(self.model.last_trig_state)) #sends sms
...
class GPIOView():
	...
	def update_alarm_mode(self):
		if (self.model.alarm_mode.__class__ in self.states_on) and self.is_active_from_state(self.model.last_trig_state.__class__):
